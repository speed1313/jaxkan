...
def test_train_parametrized_kan_layer():
    ...
    param_size = sum([width_list[l]*width_list[l+1]*coef_length for l in range(len(width_list)-1)])
    coef = jax.random.normal(jax.random.PRNGKey(0), shape=(param_size,), dtype=jnp.float32)
    print("coef:", coef)
    ...
    for l in range(len(width_list)-1):
        ...
        for i in range(width_list[l]):
            for j in range(width_list[l+1]):
                phi_l_i_j = psi(post_activation[i], t, coef, coef_idx, coef_length, k, basis_fn)
...
def B(x, k, i, t):
    ...
    def branch_false(x, t, i):
        c1 = jnp.where(t[i+k] == t[i], 0.0, (x - t[i]) / (t[i+k] - t[i]) * B(x, k-1, i, t))
        c2 = jnp.where(t[i+k+1] == t[i+1], 0.0, (t[i+k+1] - x) / (t[i+k+1] - t[i+1]) * B(x, k-1, i+1, t))
        return c1 + c2
    if k == 0:
